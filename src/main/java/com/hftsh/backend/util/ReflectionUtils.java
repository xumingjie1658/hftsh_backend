            ...
            try {
                ...
                for (Field field : fields) {
                    ...
                    if ("serialVersionUID".equals(name))
                        continue;
                    String key = com.hftsh.backend.util.StringUtils.camel2underline(name).toUpperCase();
                    String strSet = "set" + name.substring(0, 1).toUpperCase()
                            + name.substring(1, name.length());
                    Method methodSet = superClass.getDeclaredMethod(strSet,
                            clsType);
                    if (map.containsKey(key)) {
                        Object objValue = typeConversion(clsType,Converter.toString(map.get(key)));
